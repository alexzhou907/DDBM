...
class PriorAFHQv2Folder(ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        if self.pred_mode.startswith('corr-uncond'):
            ...
            prior = img + (torch.randn_like(img) * self.sigma_max) / 2 #for th
        return img, prior
...
class PriorAFHQv2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = img / 255
        prior = img + (np.random.randn(*img.shape) * self.sigma_max) / 2 #for th
