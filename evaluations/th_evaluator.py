...
def main():
    ...
    if args.metric == "fid":
        get_fid(args)
    elif args.metric == "ssim_lpips":
        get_ssim_lpips(args)

def get_fid(args):
    evaluator = FIDAndIS()
    evaluator.set_ref_batch(args.ref_batch)
    print("computing reference batch activations...")
    obj = np.load(args.ref_batch)
    if 'pool_3' in obj:
        ref_acts = obj['pool_3']
    else:
        if not ('mu' in obj and 'sigma' in obj):
            ref_acts = evaluator.read_activations(args.ref_batch)
        else:
            ref_acts= None
    # ref_acts = evaluator.read_activations(args.ref_batch)
    print("computing/reading reference batch statistics...")
    ref_stats = evaluator.read_statistics(args.ref_batch, ref_acts, store=True)
    ...
    metrics = compute_prdc(real_features=ref_acts,
                       fake_features=sample_acts,
                       nearest_k=nearest_k)
